...
func (c *Certificates) Cert(kid string, ctx context.Context) (*x509.Certificate, error) {
...
func (c *Certificates) ensureLoaded(ctx context.Context) error {
	c.RLock()
	...
	certs, cacheTime, err := download(c.URL, ctx)
	if err != nil {
		return err
	}
...
func download(url string, ctx context.Context) (map[string]*x509.Certificate, time.Duration, error) {
	//if transport == nil {
